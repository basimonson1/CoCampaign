...
When the creature is hit by an attack roll, that creature can use its [Reaction](z_Resources/CLI/rules/variant-rules/reaction-xphb.md) to roll the Bardic Inspiration die and add the number rolled to its AC against that attack, potentially causing the attack to miss.
...
You gain proficiency with Martial weapons and training with Medium armor and [Shields](z_Resources/CLI/items/shield-xphb.md).

In addition, you can use a Simple or Martial weapon as a [Spellcasting Focus](z_Resources/CLI/rules/variant-rules/spellcasting-focus-xphb.md) to cast spells from your Bard spell list.
...
After you cast a spell that has a casting time of an action, you can make one attack with a weapon as a [Bonus Action](z_Resources/CLI/rules/variant-rules/bonus-action-xphb.md).